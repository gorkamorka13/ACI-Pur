...
const { Charge } = require('../../models'); // Adjust the path to your models
const moment = require('moment');

/**
 * Génère un rapport de charges professionnel au format PDF
 * @param {Object} options - Options de configuration du rapport
 * @param {String} options.title - Titre du rapport (par défaut: 'Rapport de Charges')
 * @param {Date} options.startDate - Filtrer les charges à partir de cette date
 * @param {Date} options.endDate - Filtrer les charges jusqu'à cette date
...
async function generateReport(options = {}) {
    try {
        ...
        const totalCharge = charges.reduce((sum, charge) => sum + parseFloat(charge.montant), 0);
        const averageCharge = totalCharge / charges.length;
        const mostRecentDate = new Date(Math.max(...charges.map(charge => new Date(charge.date))));
        const oldestDate = new Date(Math.min(...charges.map(charge => new Date(charge.date))));
        ...
        charges.forEach(charge => {
            ...
            categorySummary[category] += parseFloat(charge.montant);
        });
        ...
        const doc = new PDFDocument({
            ...
            info: {
                Title: title,
                Author: 'Système de Gestion des Charges',
                Subject: 'Rapport de Charges',
                Keywords: 'charges, financier, rapport',
        ...
        throw new Error(`Échec de la génération du rapport de charges: ${error.message}`);
    }
}
...
function addSummarySection(doc, summary) {
    ...
    const summaryTable = {
        ...
        rows: [
            ['Charge Totale', formatCurrency(summary.totalCharge)],
            ['Charge Moyenne', formatCurrency(summary.averageCharge)],
...
/**
 * Ajoute un graphique à barres montrant les charges par description
...
function addDescriptionBarChart(doc, charges) {
    ...
    charges.forEach(charge => {
        const description = charge.description || 'Sans description';
        if (!descriptionData[description]) {
            descriptionData[description] = 0;
        }
        descriptionData[description] += parseFloat(charge.montant);
    });
...
/**
 * Ajoute un graphique circulaire (camembert) montrant la répartition des charges par catégorie
 * @param {PDFDocument} doc - Document PDF
