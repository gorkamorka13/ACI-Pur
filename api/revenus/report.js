...
async function generateReport(options = {}) {
    try {
        ...
        revenus.forEach(rev => {
            const category = rev.category || 'Non catégorisé';
...
function addRevenueTable(doc, revenues) {
    // Add title
    doc.fontSize(14).font('Helvetica-Bold').fillColor('#333333')
       .text('Détails des Revenus', { underline: true });
    doc.moveDown(0.5);
...
function addCategoryBreakdown(doc, categorySummary) {
    // Add section title
    doc.fontSize(14).font('Helvetica-Bold').fillColor('#333333')
       .text('Revenus par Catégorie', { underline: true });
    doc.moveDown(0.5);
...
function addDescriptionBarChart(doc, revenus) {
    // Titre de la section
    doc.fontSize(14).font('Helvetica-Bold').fillColor('#333333')
       .text('Graphique des Revenus par Description', { underline: true });
    doc.moveDown(0.5);
...
function addPieChart(doc, categorySummary) {
    // Titre de la section
    doc.fontSize(14).font('Helvetica-Bold').fillColor('#333333')
       .text('Répartition des Revenus par Catégorie', { underline: true });
    doc.moveDown(0.5);
